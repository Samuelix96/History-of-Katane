...
import { useGetKataneQuery, useGetKataneByTitleQuery, useAddKataneMutation } from '../api/apiSlice';
...
const AncientKatane = () =>
{
  ...
  const uploadToCloudinary = async (img, image2, image3, image4, image5) =>
  {
    ...
    try
    {
      const response = await fetch(`${ process.env.REACT_APP_SERVER_URL }/katanas/cloudUpload`, {
        method: 'POST',
        body: filesData
      });
  ...
  const katanasToDisplay = searchTerm ? kataByTitle?.katatitle : ancientKata?.katas || [];
