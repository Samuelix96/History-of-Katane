...
import { useGetHelmetQuery, useGetHelmetByTitleQuery, useAddHelmetMutation } from '../api/apiSlice';
...
const Helmet = () =>
{
  ...
  const uploadToCloudinary = async (img, image2, image3, image4, image5) =>
  {
    ...
    try
    {
      const response = await fetch(`${ process.env.REACT_APP_SERVER_URL }/helmets/cloudUpload`, {
        method: 'POST',
        body: filesData
      });
  ...
  const helmetsToDisplay = search ? helmetsByTitle?.helmet : helmets?.helmet || [];
